namespace L09_Corona {
    ...
    let unspParticle: unspesificParticle[] = [];
    ...
    function drawunspesificParticle(_nParticle: number): void {
        for (let drawn: number = 0; drawn < _nParticle; drawn++) {
            ...
            let particle: unspesificParticle = new unspesificParticle(position); //Position als Vektor
            particle.draw();
    ...
    function drawHumanCell(_nCell: number): void {
        ...
        for (let drawn: number = 0; drawn < _nCell; drawn++) {
            ...
            let oneCell: HumanCell = new HumanCell(positionCell); //Position als Vektor 
            oneCell.draw();
    ...
    function drawCoronaVirus(_nVirus: number): void {
        ...
        for (let drawn: number = 0; drawn < _nVirus; drawn++) {
            ...
            let oneVirus: CoronaVirus = new CoronaVirus(positionVirus); //Position als Vektor 
            oneVirus.draw();
            covid.push(oneVirus);
        }
